 ...
 * Page class
 */
class PublicServicesEdit extends PublicServices
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PublicServicesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "PublicServicesEdit";
    ...
    public function setVisibility()
    {
        ...
        $this->process_steps->setVisibility();
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("PublicServicesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "PublicServicesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "PublicServicesList") {
                            ...
                            $returnUrl = "PublicServicesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'process_steps' first before field var 'x_process_steps'
        $val = $CurrentForm->hasValue("process_steps") ? $CurrentForm->getValue("process_steps") : $CurrentForm->getValue("x_process_steps");
        if (!$this->process_steps->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->process_steps->Visible = false; // Disable update for API request
            } else {
                $this->process_steps->setFormValue($val);
            }
        }

        // Check field name 'processing_time' first before field var 'x_processing_time'
        $val = $CurrentForm->hasValue("processing_time") ? $CurrentForm->getValue("processing_time") : $CurrentForm->getValue("x_processing_time");
        if (!$this->processing_time->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->processing_time->Visible = false; // Disable update for API request
            } else {
                $this->processing_time->setFormValue($val);
            }
        }
        ...
        // Check field name 'online_link' first before field var 'x_online_link'
        $val = $CurrentForm->hasValue("online_link") ? $CurrentForm->getValue("online_link") : $CurrentForm->getValue("x_online_link");
        if (!$this->online_link->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->online_link->Visible = false; // Disable update for API request
            } else {
                $this->online_link->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->process_steps->CurrentValue = $this->process_steps->FormValue;
        $this->processing_time->CurrentValue = $this->processing_time->FormValue;
        $this->fees->CurrentValue = $this->fees->FormValue;
        $this->contact_info->CurrentValue = $this->contact_info->FormValue;
        $this->online_link->CurrentValue = $this->online_link->FormValue;
    ...
    public function renderRow()
    {
        ...
        // requirements
        $this->requirements->RowCssClass = "row";

        // process_steps
        $this->process_steps->RowCssClass = "row";

        // processing_time
        $this->processing_time->RowCssClass = "row";
        ...
        // online_link
        $this->online_link->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->process_steps->EditValue = HtmlEncode($this->process_steps->CurrentValue);
            ...
            $this->processing_time->EditValue = HtmlEncode($this->processing_time->CurrentValue);
            ...
            $this->online_link->EditValue = HtmlEncode($this->online_link->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->requirements->Visible && $this->requirements->Required) {
            ...
            if ($this->process_steps->Visible && $this->process_steps->Required) {
                if (!$this->process_steps->IsDetailKey && EmptyValue($this->process_steps->FormValue)) {
                    $this->process_steps->addErrorMessage(str_replace("%s", $this->process_steps->caption(), $this->process_steps->RequiredErrorMessage));
                }
            }
            if ($this->processing_time->Visible && $this->processing_time->Required) {
                if (!$this->processing_time->IsDetailKey && EmptyValue($this->processing_time->FormValue)) {
                    $this->processing_time->addErrorMessage(str_replace("%s", $this->processing_time->caption(), $this->processing_time->RequiredErrorMessage));
                }
            }
            if ($this->fees->Visible && $this->fees->Required) {
            ...
            if ($this->online_link->Visible && $this->online_link->Required) {
                if (!$this->online_link->IsDetailKey && EmptyValue($this->online_link->FormValue)) {
                    $this->online_link->addErrorMessage(str_replace("%s", $this->online_link->caption(), $this->online_link->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // requirements
        $this->requirements->setDbValueDef($rsnew, $this->requirements->CurrentValue, $this->requirements->ReadOnly);

        // process_steps
        $this->process_steps->setDbValueDef($rsnew, $this->process_steps->CurrentValue, $this->process_steps->ReadOnly);

        // processing_time
        $this->processing_time->setDbValueDef($rsnew, $this->processing_time->CurrentValue, $this->processing_time->ReadOnly);

        // fees
        $this->fees->setDbValueDef($rsnew, $this->fees->CurrentValue, $this->fees->ReadOnly);

        // contact_info
        $this->contact_info->setDbValueDef($rsnew, $this->contact_info->CurrentValue, $this->contact_info->ReadOnly);

        // online_link
        $this->online_link->setDbValueDef($rsnew, $this->online_link->CurrentValue, $this->online_link->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['requirements'])) { // requirements
            $this->requirements->CurrentValue = $row['requirements'];
        }
        if (isset($row['process_steps'])) { // process_steps
            $this->process_steps->CurrentValue = $row['process_steps'];
        }
        if (isset($row['processing_time'])) { // processing_time
            $this->processing_time->CurrentValue = $row['processing_time'];
        }
        if (isset($row['fees'])) { // fees
            $this->fees->CurrentValue = $row['fees'];
        }
        if (isset($row['contact_info'])) { // contact_info
            $this->contact_info->CurrentValue = $row['contact_info'];
        }
        if (isset($row['online_link'])) { // online_link
            $this->online_link->CurrentValue = $row['online_link'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PublicServicesList"), "", $this->TableVar, true);
        $pageId = "edit";
