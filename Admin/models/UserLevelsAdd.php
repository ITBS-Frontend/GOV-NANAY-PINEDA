...
class UserLevelsAdd extends UserLevels
{
    ...
    {
        $this->hierarchy->DefaultValue = $this->hierarchy->getDefault(); // PHP
        $this->hierarchy->OldValue = $this->hierarchy->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'hierarchy' first before field var 'x_hierarchy'
        $val = $CurrentForm->hasValue("hierarchy") ? $CurrentForm->getValue("hierarchy") : $CurrentForm->getValue("x_hierarchy");
        if (!$this->hierarchy->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->hierarchy->Visible = false; // Disable update for API request
            } else {
                $this->hierarchy->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->hierarchy->EditValue = $this->hierarchy->CurrentValue;
            $this->hierarchy->PlaceHolder = RemoveHtml($this->hierarchy->caption());
            if (strval($this->hierarchy->EditValue) != "" && is_numeric($this->hierarchy->EditValue)) {
                $this->hierarchy->EditValue = FormatNumber($this->hierarchy->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->user_level_name->Visible && $this->user_level_name->Required) {
            ...
            if ($this->hierarchy->Visible && $this->hierarchy->Required) {
                if (!$this->hierarchy->IsDetailKey && EmptyValue($this->hierarchy->FormValue)) {
                    $this->hierarchy->addErrorMessage(str_replace("%s", $this->hierarchy->caption(), $this->hierarchy->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->hierarchy->FormValue)) {
                $this->hierarchy->addErrorMessage($this->hierarchy->getErrorMessage(false));
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        } elseif ((int)$this->user_level_id->CurrentValue == AdvancedSecurity::DEFAULT_USER_LEVEL_ID && !SameText($this->user_level_name->CurrentValue, "Default")) {
            $this->setFailureMessage($Language->phrase("UserLevelDefaultName"));
        } elseif ((int)$this->user_level_id->CurrentValue == AdvancedSecurity::ADMIN_USER_LEVEL_ID && !SameText($this->user_level_name->CurrentValue, "Administrator")) {
            $this->setFailureMessage($Language->phrase("UserLevelAdministratorName"));
        } elseif ((int)$this->user_level_id->CurrentValue == AdvancedSecurity::ANONYMOUS_USER_LEVEL_ID && !SameText($this->user_level_name->CurrentValue, "Anonymous")) {
            $this->setFailureMessage($Language->phrase("UserLevelAnonymousName"));
        } elseif ((int)$this->user_level_id->CurrentValue > AdvancedSecurity::DEFAULT_USER_LEVEL_ID && in_array(strtolower(trim($this->user_level_name->CurrentValue)), ["anonymous", "administrator", "default"])) {
    ...
    protected function getAddRow()
    {
        ...
        // hierarchy
        $this->hierarchy->setDbValueDef($rsnew, $this->hierarchy->CurrentValue, strval($this->hierarchy->CurrentValue) == "");
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['user_level_name'])) { // user_level_name
            $this->user_level_name->setFormValue($row['user_level_name']);
        }
        if (isset($row['hierarchy'])) { // hierarchy
            $this->hierarchy->setFormValue($row['hierarchy']);
        }
