 ...
 * Page class
 */
class NewsPostTagsEdit extends NewsPostTags
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NewsPostTagsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "NewsPostTagsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("post_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->post_id->setQueryStringValue($keyValue);
                $this->post_id->setOldValue($this->post_id->QueryStringValue);
            } elseif (Post("post_id") !== null) {
                $this->post_id->setFormValue(Post("post_id"));
                $this->post_id->setOldValue($this->post_id->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("tag_id") ?? Key(1) ?? Route(3)) !== null) {
                $this->tag_id->setQueryStringValue($keyValue);
                $this->tag_id->setOldValue($this->tag_id->QueryStringValue);
            } elseif (Post("tag_id") !== null) {
                $this->tag_id->setFormValue(Post("tag_id"));
                $this->tag_id->setOldValue($this->tag_id->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("post_id") ?? Route("post_id")) !== null) {
                    $this->post_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->post_id->CurrentValue = null;
                }
                if (($keyValue = Get("tag_id") ?? Route("tag_id")) !== null) {
                    $this->tag_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->tag_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("NewsPostTagsList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "NewsPostTagsList") {
                            ...
                            $returnUrl = "NewsPostTagsList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    public function renderRow()
    {
        ...
        // post_id
        $this->post_id->RowCssClass = "row";

        // tag_id
        $this->tag_id->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("NewsPostTagsList"), "", $this->TableVar, true);
        $pageId = "edit";
