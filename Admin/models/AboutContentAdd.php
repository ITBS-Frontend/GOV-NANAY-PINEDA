 ...
 * Page class
 */
class AboutContentAdd extends AboutContent
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AboutContentAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "AboutContentAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("AboutContentList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "AboutContentList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "AboutContentView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AboutContentList") {
                            ...
                            $returnUrl = "AboutContentList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'section_type' first before field var 'x_section_type'
        $val = $CurrentForm->hasValue("section_type") ? $CurrentForm->getValue("section_type") : $CurrentForm->getValue("x_section_type");
        if (!$this->section_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->section_type->Visible = false; // Disable update for API request
            } else {
                $this->section_type->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->section_type->CurrentValue = $this->section_type->FormValue;
    ...
    public function renderRow()
    {
        ...
        // section_type
        $this->section_type->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // section_type
            $this->section_type->setupEditAttributes();
            if (!$this->section_type->Raw) {
                $this->section_type->CurrentValue = HtmlDecode($this->section_type->CurrentValue);
            }
            $this->section_type->EditValue = HtmlEncode($this->section_type->CurrentValue);
            $this->section_type->PlaceHolder = RemoveHtml($this->section_type->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->section_type->Visible && $this->section_type->Required) {
                if (!$this->section_type->IsDetailKey && EmptyValue($this->section_type->FormValue)) {
                    $this->section_type->addErrorMessage(str_replace("%s", $this->section_type->caption(), $this->section_type->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // section_type
        $this->section_type->setDbValueDef($rsnew, $this->section_type->CurrentValue, false);
    ...
    {
        if (isset($row['section_type'])) { // section_type
            $this->section_type->setFormValue($row['section_type']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AboutContentList"), "", $this->TableVar, true);
