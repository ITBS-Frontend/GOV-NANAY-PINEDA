 ...
 * Page class
 */
class DifficultyLevelsEdit extends DifficultyLevels
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DifficultyLevelsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "DifficultyLevelsEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("DifficultyLevelsList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "DifficultyLevelsList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "DifficultyLevelsList") {
                            ...
                            $returnUrl = "DifficultyLevelsList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'level_name' first before field var 'x_level_name'
        $val = $CurrentForm->hasValue("level_name") ? $CurrentForm->getValue("level_name") : $CurrentForm->getValue("x_level_name");
        if (!$this->level_name->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->level_name->Visible = false; // Disable update for API request
            } else {
                $this->level_name->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // level_name
        $this->level_name->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // level_name
            $this->level_name->setupEditAttributes();
            if (!$this->level_name->Raw) {
                $this->level_name->CurrentValue = HtmlDecode($this->level_name->CurrentValue);
            }
            $this->level_name->EditValue = HtmlEncode($this->level_name->CurrentValue);
            $this->level_name->PlaceHolder = RemoveHtml($this->level_name->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->level_name->Visible && $this->level_name->Required) {
                if (!$this->level_name->IsDetailKey && EmptyValue($this->level_name->FormValue)) {
                    $this->level_name->addErrorMessage(str_replace("%s", $this->level_name->caption(), $this->level_name->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (level_name)
        if ($this->level_name->CurrentValue != "") {
            $filterChk = "(\"level_name\" = '" . AdjustSql($this->level_name->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->level_name->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->level_name->CurrentValue, $idxErrMsg);
    ...
    {
        if (isset($row['level_name'])) { // level_name
            $this->level_name->CurrentValue = $row['level_name'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("DifficultyLevelsList"), "", $this->TableVar, true);
        $pageId = "edit";
