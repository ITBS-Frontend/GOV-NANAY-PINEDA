...
class NewsPostsAdd extends NewsPosts
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("news_post_tags", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("NewsPostTagsGrid");
            if (in_array("news_post_tags", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->post_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "news_post_tags"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->post_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
