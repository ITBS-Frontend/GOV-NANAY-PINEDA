 ...
 * Page class
 */
class EconomicIndicatorsAdd extends EconomicIndicators
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EconomicIndicatorsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "EconomicIndicatorsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("EconomicIndicatorsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "EconomicIndicatorsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "EconomicIndicatorsView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "EconomicIndicatorsList") {
                            ...
                            $returnUrl = "EconomicIndicatorsList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'quarter' first before field var 'x_quarter'
        $val = $CurrentForm->hasValue("quarter") ? $CurrentForm->getValue("quarter") : $CurrentForm->getValue("x_quarter");
        if (!$this->quarter->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->quarter->Visible = false; // Disable update for API request
            } else {
                $this->quarter->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->quarter->CurrentValue = $this->quarter->FormValue;
    ...
    public function renderRow()
    {
        ...
        // indicator_name
        $this->indicator_name->RowCssClass = "row";
        ...
        // quarter
        $this->quarter->RowCssClass = "row";

        // source
        $this->source->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // indicator_name
            $this->indicator_name->setupEditAttributes();
            ...
            // quarter
            $this->quarter->setupEditAttributes();
            if (!$this->quarter->Raw) {
                $this->quarter->CurrentValue = HtmlDecode($this->quarter->CurrentValue);
            }
            $this->quarter->EditValue = HtmlEncode($this->quarter->CurrentValue);
            $this->quarter->PlaceHolder = RemoveHtml($this->quarter->caption());

            // source
            $this->source->setupEditAttributes();
            if (!$this->source->Raw) {
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->indicator_name->Visible && $this->indicator_name->Required) {
                if (!$this->indicator_name->IsDetailKey && EmptyValue($this->indicator_name->FormValue)) {
                    $this->indicator_name->addErrorMessage(str_replace("%s", $this->indicator_name->caption(), $this->indicator_name->RequiredErrorMessage));
                }
            }
            ...
            if ($this->year->Visible && $this->year->Required) {
            ...
            if ($this->quarter->Visible && $this->quarter->Required) {
                if (!$this->quarter->IsDetailKey && EmptyValue($this->quarter->FormValue)) {
                    $this->quarter->addErrorMessage(str_replace("%s", $this->quarter->caption(), $this->quarter->RequiredErrorMessage));
                }
            }
            if ($this->source->Visible && $this->source->Required) {
                if (!$this->source->IsDetailKey && EmptyValue($this->source->FormValue)) {
                    $this->source->addErrorMessage(str_replace("%s", $this->source->caption(), $this->source->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // indicator_name
        $this->indicator_name->setDbValueDef($rsnew, $this->indicator_name->CurrentValue, false);
        ...
        // year
        $this->year->setDbValueDef($rsnew, $this->year->CurrentValue, false);

        // quarter
        $this->quarter->setDbValueDef($rsnew, $this->quarter->CurrentValue, false);
    ...
    {
        if (isset($row['indicator_name'])) { // indicator_name
            $this->indicator_name->setFormValue($row['indicator_name']);
        }
        ...
        if (isset($row['year'])) { // year
            $this->year->setFormValue($row['year']);
        }
        if (isset($row['quarter'])) { // quarter
            $this->quarter->setFormValue($row['quarter']);
        }
        if (isset($row['source'])) { // source
            $this->source->setFormValue($row['source']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("EconomicIndicatorsList"), "", $this->TableVar, true);
