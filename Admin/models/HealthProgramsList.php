 ...
 * Page class
 */
class HealthProgramsList extends HealthPrograms
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HealthProgramsList";
    ...
    public $FormName = "fhealth_programslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "HealthProgramsList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "HealthProgramsAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "HealthProgramsDelete";
        $this->MultiUpdateUrl = "HealthProgramsUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fhealth_programsgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->target_beneficiaries->AdvancedSearch->toJson(), ","); // Field target_beneficiaries
        $filterList = Concat($filterList, $this->coverage_area->AdvancedSearch->toJson(), ","); // Field coverage_area
        $filterList = Concat($filterList, $this->implementation_date->AdvancedSearch->toJson(), ","); // Field implementation_date
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fhealth_programssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->target_beneficiaries->AdvancedSearch->SearchValue = @$filter["x_target_beneficiaries"];
        $this->target_beneficiaries->AdvancedSearch->SearchOperator = @$filter["z_target_beneficiaries"];
        $this->target_beneficiaries->AdvancedSearch->SearchCondition = @$filter["v_target_beneficiaries"];
        $this->target_beneficiaries->AdvancedSearch->SearchValue2 = @$filter["y_target_beneficiaries"];
        $this->target_beneficiaries->AdvancedSearch->SearchOperator2 = @$filter["w_target_beneficiaries"];
        $this->target_beneficiaries->AdvancedSearch->save();

        // Field coverage_area
        $this->coverage_area->AdvancedSearch->SearchValue = @$filter["x_coverage_area"];
        $this->coverage_area->AdvancedSearch->SearchOperator = @$filter["z_coverage_area"];
        $this->coverage_area->AdvancedSearch->SearchCondition = @$filter["v_coverage_area"];
        $this->coverage_area->AdvancedSearch->SearchValue2 = @$filter["y_coverage_area"];
        $this->coverage_area->AdvancedSearch->SearchOperator2 = @$filter["w_coverage_area"];
        $this->coverage_area->AdvancedSearch->save();

        // Field implementation_date
        $this->implementation_date->AdvancedSearch->SearchValue = @$filter["x_implementation_date"];
        $this->implementation_date->AdvancedSearch->SearchOperator = @$filter["z_implementation_date"];
        $this->implementation_date->AdvancedSearch->SearchCondition = @$filter["v_implementation_date"];
        $this->implementation_date->AdvancedSearch->SearchValue2 = @$filter["y_implementation_date"];
        $this->implementation_date->AdvancedSearch->SearchOperator2 = @$filter["w_implementation_date"];
        $this->implementation_date->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->target_beneficiaries;
        $searchFlds[] = &$this->coverage_area;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->target_beneficiaries); // target_beneficiaries
            $this->updateSort($this->coverage_area); // coverage_area
            $this->updateSort($this->implementation_date); // implementation_date
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->target_beneficiaries->setSort("");
                $this->coverage_area->setSort("");
                $this->implementation_date->setSort("");
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"health_programs\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"health_programs\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"health_programs\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fhealth_programslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fhealth_programslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"health_programs\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "target_beneficiaries");
            $this->createColumnOption($option, "coverage_area");
            $this->createColumnOption($option, "implementation_date");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhealth_programssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhealth_programssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fhealth_programslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_health_programs", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_health_programs",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->target_beneficiaries->TooltipValue = "";

            // coverage_area
            $this->coverage_area->HrefValue = "";
            $this->coverage_area->TooltipValue = "";
            ...
            if ($this->featured_image->UseColorbox) {
                ...
                $this->featured_image->LinkAttrs["data-rel"] = "health_programs_x" . $this->RowCount . "_featured_image";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fhealth_programssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
