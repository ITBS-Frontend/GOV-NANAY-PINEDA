 ...
 * Page class
 */
class ProjectHighlightsAdd extends ProjectHighlights
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProjectHighlightsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ProjectHighlightsAdd";
    ...
    public function setVisibility()
    {
        ...
        $this->highlight_text->setVisibility();
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ProjectHighlightsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ProjectHighlightsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ProjectHighlightsView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ProjectHighlightsList") {
                            ...
                            $returnUrl = "ProjectHighlightsList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'highlight_text' first before field var 'x_highlight_text'
        $val = $CurrentForm->hasValue("highlight_text") ? $CurrentForm->getValue("highlight_text") : $CurrentForm->getValue("x_highlight_text");
        if (!$this->highlight_text->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->highlight_text->Visible = false; // Disable update for API request
            } else {
                $this->highlight_text->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->highlight_text->CurrentValue = $this->highlight_text->FormValue;
    ...
    public function renderRow()
    {
        ...
        // highlight_text
        $this->highlight_text->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->highlight_text->EditValue = HtmlEncode($this->highlight_text->CurrentValue);
        ...
        $validateForm = true;
            ...
            if ($this->highlight_text->Visible && $this->highlight_text->Required) {
                if (!$this->highlight_text->IsDetailKey && EmptyValue($this->highlight_text->FormValue)) {
                    $this->highlight_text->addErrorMessage(str_replace("%s", $this->highlight_text->caption(), $this->highlight_text->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // highlight_text
        $this->highlight_text->setDbValueDef($rsnew, $this->highlight_text->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['highlight_text'])) { // highlight_text
            $this->highlight_text->setFormValue($row['highlight_text']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ProjectHighlightsList"), "", $this->TableVar, true);
