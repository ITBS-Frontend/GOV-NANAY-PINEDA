...
class UserLevels extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->user_level_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['user_level_id'] = &$this->user_level_id;

        // user_level_name
        $this->user_level_name = new DbField(
            $this, // Table
            'x_user_level_name', // Variable name
            'user_level_name', // Name
            '"user_level_name"', // Expression
            '"user_level_name"', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"user_level_name"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->user_level_name->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        $this->hierarchy = new DbField(
            ...
            'CAST("hierarchy" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
        ...
        $this->hierarchy->addMethod("getDefault", fn() => 0);
        $this->hierarchy->InputTextType = "text";
        $this->hierarchy->Raw = true;
        $this->hierarchy->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->hierarchy->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->user_level_id->OldValue) && !$current ? $this->user_level_id->OldValue : $this->user_level_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    {
        $json = "";
        $json .= "\"user_level_id\":" . VarToJson($this->user_level_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
