...
/**
 * Table class for province_history
 */
class ProvinceHistory extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "province_history";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->period->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        // timeline_year
        $this->timeline_year = new DbField(
            $this, // Table
            'x_timeline_year', // Variable name
            'timeline_year', // Name
            '"timeline_year"', // Expression
            'CAST("timeline_year" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"timeline_year"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->timeline_year->InputTextType = "text";
        $this->timeline_year->Raw = true;
        $this->timeline_year->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->timeline_year->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['timeline_year'] = &$this->timeline_year;
        ...
        $this->is_active->Lookup = new Lookup($this->is_active, 'province_history', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "province_history";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ProvinceHistoryList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ProvinceHistoryView" => $Language->phrase("View"),
            "ProvinceHistoryEdit" => $Language->phrase("Edit"),
            "ProvinceHistoryAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ProvinceHistoryView",
            Config("API_ADD_ACTION") => "ProvinceHistoryAdd",
            Config("API_EDIT_ACTION") => "ProvinceHistoryEdit",
            Config("API_DELETE_ACTION") => "ProvinceHistoryDelete",
            Config("API_LIST_ACTION") => "ProvinceHistoryList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ProvinceHistoryView", $parm);
        } else {
            $url = $this->keyUrl("ProvinceHistoryView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ProvinceHistoryAdd?" . $parm;
        } else {
            $url = "ProvinceHistoryAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProvinceHistoryEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProvinceHistoryList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProvinceHistoryAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProvinceHistoryList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ProvinceHistoryDelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->timeline_year);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->timeline_year);
