 ...
 * Page class
 */
class EmergencyFacilitiesEdit extends EmergencyFacilities
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmergencyFacilitiesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "EmergencyFacilitiesEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("EmergencyFacilitiesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "EmergencyFacilitiesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "EmergencyFacilitiesList") {
                            ...
                            $returnUrl = "EmergencyFacilitiesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'capacity' first before field var 'x_capacity'
        $val = $CurrentForm->hasValue("capacity") ? $CurrentForm->getValue("capacity") : $CurrentForm->getValue("x_capacity");
        if (!$this->capacity->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->capacity->Visible = false; // Disable update for API request
            } else {
                $this->capacity->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // capacity
            $this->capacity->setupEditAttributes();
            $this->capacity->EditValue = $this->capacity->CurrentValue;
            $this->capacity->PlaceHolder = RemoveHtml($this->capacity->caption());
            if (strval($this->capacity->EditValue) != "" && is_numeric($this->capacity->EditValue)) {
                $this->capacity->EditValue = FormatNumber($this->capacity->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->capacity->Visible && $this->capacity->Required) {
                if (!$this->capacity->IsDetailKey && EmptyValue($this->capacity->FormValue)) {
                    $this->capacity->addErrorMessage(str_replace("%s", $this->capacity->caption(), $this->capacity->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->capacity->FormValue)) {
                $this->capacity->addErrorMessage($this->capacity->getErrorMessage(false));
            }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['capacity'])) { // capacity
            $this->capacity->CurrentValue = $row['capacity'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("EmergencyFacilitiesList"), "", $this->TableVar, true);
        $pageId = "edit";
