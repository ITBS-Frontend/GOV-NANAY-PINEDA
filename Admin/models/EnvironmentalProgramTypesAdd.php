 ...
 * Page class
 */
class EnvironmentalProgramTypesAdd extends EnvironmentalProgramTypes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EnvironmentalProgramTypesAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "EnvironmentalProgramTypesAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("EnvironmentalProgramTypesList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "EnvironmentalProgramTypesList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "EnvironmentalProgramTypesView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "EnvironmentalProgramTypesList") {
                            ...
                            $returnUrl = "EnvironmentalProgramTypesList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'type_name' first before field var 'x_type_name'
        $val = $CurrentForm->hasValue("type_name") ? $CurrentForm->getValue("type_name") : $CurrentForm->getValue("x_type_name");
        if (!$this->type_name->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->type_name->Visible = false; // Disable update for API request
            } else {
                $this->type_name->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // type_name
        $this->type_name->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->type_name->EditValue = HtmlEncode($this->type_name->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->type_name->Visible && $this->type_name->Required) {
                if (!$this->type_name->IsDetailKey && EmptyValue($this->type_name->FormValue)) {
                    $this->type_name->addErrorMessage(str_replace("%s", $this->type_name->caption(), $this->type_name->RequiredErrorMessage));
                }
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->type_name->CurrentValue != "") { // Check field with unique index
            $filter = "(\"type_name\" = '" . AdjustSql($this->type_name->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->type_name->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->type_name->CurrentValue, $idxErrMsg);
    ...
    {
        if (isset($row['type_name'])) { // type_name
            $this->type_name->setFormValue($row['type_name']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("EnvironmentalProgramTypesList"), "", $this->TableVar, true);
