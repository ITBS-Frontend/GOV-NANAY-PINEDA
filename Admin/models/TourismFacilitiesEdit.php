 ...
 * Page class
 */
class TourismFacilitiesEdit extends TourismFacilities
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TourismFacilitiesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "TourismFacilitiesEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("TourismFacilitiesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "TourismFacilitiesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "TourismFacilitiesList") {
                            ...
                            $returnUrl = "TourismFacilitiesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'price_range' first before field var 'x_price_range'
        $val = $CurrentForm->hasValue("price_range") ? $CurrentForm->getValue("price_range") : $CurrentForm->getValue("x_price_range");
        if (!$this->price_range->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->price_range->Visible = false; // Disable update for API request
            } else {
                $this->price_range->setFormValue($val);
            }
        }

        // Check field name 'amenities' first before field var 'x_amenities'
        $val = $CurrentForm->hasValue("amenities") ? $CurrentForm->getValue("amenities") : $CurrentForm->getValue("x_amenities");
        if (!$this->amenities->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->amenities->Visible = false; // Disable update for API request
            } else {
                $this->amenities->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // price_range
        $this->price_range->RowCssClass = "row";

        // amenities
        $this->amenities->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->price_range->EditValue = HtmlEncode($this->price_range->CurrentValue);
            $this->price_range->PlaceHolder = RemoveHtml($this->price_range->caption());

            // amenities
            $this->amenities->setupEditAttributes();
            $this->amenities->EditValue = HtmlEncode($this->amenities->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->price_range->Visible && $this->price_range->Required) {
                if (!$this->price_range->IsDetailKey && EmptyValue($this->price_range->FormValue)) {
                    $this->price_range->addErrorMessage(str_replace("%s", $this->price_range->caption(), $this->price_range->RequiredErrorMessage));
                }
            }
            if ($this->amenities->Visible && $this->amenities->Required) {
                if (!$this->amenities->IsDetailKey && EmptyValue($this->amenities->FormValue)) {
                    $this->amenities->addErrorMessage(str_replace("%s", $this->amenities->caption(), $this->amenities->RequiredErrorMessage));
                }
            }
            ...
            if (!CheckNumber($this->rating->FormValue)) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // amenities
        $this->amenities->setDbValueDef($rsnew, $this->amenities->CurrentValue, $this->amenities->ReadOnly);
        ...
        // facility_type_id
        $this->facility_type_id->setDbValueDef($rsnew, $this->facility_type_id->CurrentValue, $this->facility_type_id->ReadOnly);

        // ownership_type_id
        $this->ownership_type_id->setDbValueDef($rsnew, $this->ownership_type_id->CurrentValue, $this->ownership_type_id->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['price_range'])) { // price_range
            $this->price_range->CurrentValue = $row['price_range'];
        }
        if (isset($row['amenities'])) { // amenities
            $this->amenities->CurrentValue = $row['amenities'];
        }
        ...
        if (isset($row['facility_type_id'])) { // facility_type_id
            $this->facility_type_id->CurrentValue = $row['facility_type_id'];
        }
        if (isset($row['ownership_type_id'])) { // ownership_type_id
            $this->ownership_type_id->CurrentValue = $row['ownership_type_id'];
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TourismFacilitiesList"), "", $this->TableVar, true);
        $pageId = "edit";
