 ...
 * Page class
 */
class ProfileImagesAdd extends ProfileImages
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProfileImagesAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ProfileImagesAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ProfileImagesList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ProfileImagesList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ProfileImagesView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ProfileImagesList") {
                            ...
                            $returnUrl = "ProfileImagesList"; // Return list page content
                        }
                    }
    ...
    protected function getUploadFiles()
    {
        ...
        $this->image_type->CurrentValue = $this->image_path->Upload->FileName;
    }

    // Load default values
    protected function loadDefaultValues()
    {
        $this->image_type->DefaultValue = $this->image_type->getDefault(); // PHP
        $this->image_type->OldValue = $this->image_type->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'image_type' first before field var 'x_image_type'
        $val = $CurrentForm->hasValue("image_type") ? $CurrentForm->getValue("image_type") : $CurrentForm->getValue("x_image_type");
        if (!$this->image_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->image_type->Visible = false; // Disable update for API request
            } else {
                $this->image_type->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // image_path
        $this->image_path->RowCssClass = "row";

        // image_type
        $this->image_type->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->isShow() || $this->isCopy()) {
                RenderUploadField($this->image_path);
            }
            ...
            $this->image_type->EditValue = HtmlEncode($this->image_type->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->image_type->Visible && $this->image_type->Required) {
                if (!$this->image_type->IsDetailKey && EmptyValue($this->image_type->FormValue)) {
                    $this->image_type->addErrorMessage(str_replace("%s", $this->image_type->caption(), $this->image_type->RequiredErrorMessage));
                }
            }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['image_type'])) { // image_type
            $this->image_type->setFormValue($row['image_type']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ProfileImagesList"), "", $this->TableVar, true);
