 ...
 * Page class
 */
class ProvinceHistoryAdd extends ProvinceHistory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProvinceHistoryAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ProvinceHistoryAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ProvinceHistoryList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ProvinceHistoryList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ProvinceHistoryView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ProvinceHistoryList") {
                            ...
                            $returnUrl = "ProvinceHistoryList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'period' first before field var 'x_period'
        $val = $CurrentForm->hasValue("period") ? $CurrentForm->getValue("period") : $CurrentForm->getValue("x_period");
        if (!$this->period->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->period->Visible = false; // Disable update for API request
            } else {
                $this->period->setFormValue($val);
            }
        }
        ...
        // Check field name 'timeline_year' first before field var 'x_timeline_year'
        $val = $CurrentForm->hasValue("timeline_year") ? $CurrentForm->getValue("timeline_year") : $CurrentForm->getValue("x_timeline_year");
        if (!$this->timeline_year->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->timeline_year->Visible = false; // Disable update for API request
            } else {
                $this->timeline_year->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // timeline_year
        $this->timeline_year->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->period->EditValue = HtmlEncode($this->period->CurrentValue);
            ...
            // timeline_year
            $this->timeline_year->setupEditAttributes();
            $this->timeline_year->EditValue = $this->timeline_year->CurrentValue;
            $this->timeline_year->PlaceHolder = RemoveHtml($this->timeline_year->caption());
            if (strval($this->timeline_year->EditValue) != "" && is_numeric($this->timeline_year->EditValue)) {
                $this->timeline_year->EditValue = FormatNumber($this->timeline_year->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->period->Visible && $this->period->Required) {
                if (!$this->period->IsDetailKey && EmptyValue($this->period->FormValue)) {
                    $this->period->addErrorMessage(str_replace("%s", $this->period->caption(), $this->period->RequiredErrorMessage));
                }
            }
            ...
            if ($this->timeline_year->Visible && $this->timeline_year->Required) {
                if (!$this->timeline_year->IsDetailKey && EmptyValue($this->timeline_year->FormValue)) {
                    $this->timeline_year->addErrorMessage(str_replace("%s", $this->timeline_year->caption(), $this->timeline_year->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->timeline_year->FormValue)) {
    ...
    protected function getAddRow()
    {
        ...
        // period
        $this->period->setDbValueDef($rsnew, $this->period->CurrentValue, false);

        // content
        $this->_content->setDbValueDef($rsnew, $this->_content->CurrentValue, false);

        // timeline_year
        $this->timeline_year->setDbValueDef($rsnew, $this->timeline_year->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['period'])) { // period
            $this->period->setFormValue($row['period']);
        }
        if (isset($row['content'])) { // content
            $this->_content->setFormValue($row['content']);
        }
        if (isset($row['timeline_year'])) { // timeline_year
            $this->timeline_year->setFormValue($row['timeline_year']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ProvinceHistoryList"), "", $this->TableVar, true);
