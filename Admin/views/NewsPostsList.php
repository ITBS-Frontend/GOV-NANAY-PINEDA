...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fnews_postssrch" id="fnews_postssrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
<div id="fnews_postssrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { news_posts: currentTable } });
var currentForm;
var fnews_postssrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="fnews_postssrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="fnews_postssrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="fnews_postssrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="fnews_postssrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_news_posts" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_news_postslist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Page->id->Visible) { // id ?>
        <th data-name="id" class="<?= $Page->id->headerCellClass() ?>"><div id="elh_news_posts_id" class="news_posts_id"><?= $Page->renderFieldHeader($Page->id) ?></div></th>
<?php } ?>
...
<?php if ($Page->featured_image->Visible) { // featured_image ?>
        <th data-name="featured_image" class="<?= $Page->featured_image->headerCellClass() ?>"><div id="elh_news_posts_featured_image" class="news_posts_featured_image"><?= $Page->renderFieldHeader($Page->featured_image) ?></div></th>
<?php } ?>
<?php if ($Page->author_name->Visible) { // author_name ?>
        <th data-name="author_name" class="<?= $Page->author_name->headerCellClass() ?>"><div id="elh_news_posts_author_name" class="news_posts_author_name"><?= $Page->renderFieldHeader($Page->author_name) ?></div></th>
<?php } ?>
<?php if ($Page->is_featured->Visible) { // is_featured ?>
        <th data-name="is_featured" class="<?= $Page->is_featured->headerCellClass() ?>"><div id="elh_news_posts_is_featured" class="news_posts_is_featured"><?= $Page->renderFieldHeader($Page->is_featured) ?></div></th>
<?php } ?>
<?php if ($Page->is_published->Visible) { // is_published ?>
        <th data-name="is_published" class="<?= $Page->is_published->headerCellClass() ?>"><div id="elh_news_posts_is_published" class="news_posts_is_published"><?= $Page->renderFieldHeader($Page->is_published) ?></div></th>
<?php } ?>
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_news_posts_id" class="el_news_posts_id">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_news_posts_category_id" class="el_news_posts_category_id">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_news_posts_featured_image" class="el_news_posts_featured_image">
<span>
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_news_posts_author_name" class="el_news_posts_author_name">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_news_posts_is_featured" class="el_news_posts_is_featured">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_news_posts_is_published" class="el_news_posts_is_published">
