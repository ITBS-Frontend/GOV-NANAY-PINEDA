...
$Page->showMessage();
?>
<form name="fnews_post_tagsadd" id="fnews_post_tagsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span class="form-control-plaintext"><?= $Page->post_id->getDisplayValue($Page->post_id->ViewValue) ?></span></span>
...
<span id="el_news_post_tags_post_id">
    <select
        ...
        data-select2-id="fnews_post_tagsadd_x_post_id"
        <?php } ?>
...
<?php if (!$Page->post_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fnews_post_tagsadd", function() {
    var options = { name: "x_post_id", selectId: "fnews_post_tagsadd_x_post_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fnews_post_tagsadd.lists.post_id?.lookupOptions.length) {
        options.data = { id: "x_post_id", form: "fnews_post_tagsadd" };
    } else {
        options.ajax = { id: "x_post_id", form: "fnews_post_tagsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_news_post_tags_tag_id">
    <select
        ...
        data-select2-id="fnews_post_tagsadd_x_tag_id"
        <?php } ?>
        data-table="news_post_tags"
        data-field="x_tag_id"
...
<script>
loadjs.ready("fnews_post_tagsadd", function() {
    var options = { name: "x_tag_id", selectId: "fnews_post_tagsadd_x_tag_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fnews_post_tagsadd.lists.tag_id?.lookupOptions.length) {
        options.data = { id: "x_tag_id", form: "fnews_post_tagsadd" };
    } else {
        options.ajax = { id: "x_tag_id", form: "fnews_post_tagsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fnews_post_tagsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fnews_post_tagsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
