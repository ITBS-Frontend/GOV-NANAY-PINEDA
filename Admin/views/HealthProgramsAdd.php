...
$Page->showMessage();
?>
<form name="fhealth_programsadd" id="fhealth_programsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready(["fhealth_programsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fhealth_programsadd", "x_implementation_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_health_programs_status_id">
    <select
        ...
        data-select2-id="fhealth_programsadd_x_status_id"
        <?php } ?>
        data-table="health_programs"
...
<?php if (!$Page->status_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fhealth_programsadd", function() {
    var options = { name: "x_status_id", selectId: "fhealth_programsadd_x_status_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhealth_programsadd.lists.status_id?.lookupOptions.length) {
        options.data = { id: "x_status_id", form: "fhealth_programsadd" };
    } else {
        options.ajax = { id: "x_status_id", form: "fhealth_programsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fhealth_programsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fhealth_programsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
