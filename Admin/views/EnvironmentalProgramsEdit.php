...
<span id="el_environmental_programs_program_type_id">
    <select
        ...
        data-select2-id="fenvironmental_programsedit_x_program_type_id"
        <?php } ?>
        data-table="environmental_programs"
...
<?php if (!$Page->program_type_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fenvironmental_programsedit", function() {
    var options = { name: "x_program_type_id", selectId: "fenvironmental_programsedit_x_program_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fenvironmental_programsedit.lists.program_type_id?.lookupOptions.length) {
        options.data = { id: "x_program_type_id", form: "fenvironmental_programsedit" };
    } else {
        options.ajax = { id: "x_program_type_id", form: "fenvironmental_programsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_environmental_programs_status_id">
    <select
        ...
        data-select2-id="fenvironmental_programsedit_x_status_id"
        <?php } ?>
        data-table="environmental_programs"
...
<script>
loadjs.ready("fenvironmental_programsedit", function() {
    var options = { name: "x_status_id", selectId: "fenvironmental_programsedit_x_status_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fenvironmental_programsedit.lists.status_id?.lookupOptions.length) {
        options.data = { id: "x_status_id", form: "fenvironmental_programsedit" };
    } else {
        options.ajax = { id: "x_status_id", form: "fenvironmental_programsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
