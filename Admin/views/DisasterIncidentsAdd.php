...
$Page->showMessage();
?>
<form name="fdisaster_incidentsadd" id="fdisaster_incidentsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready(["fdisaster_incidentsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdisaster_incidentsadd", "x_occurrence_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_disaster_incidents_incident_type_id">
    <select
        ...
        data-select2-id="fdisaster_incidentsadd_x_incident_type_id"
        <?php } ?>
...
<?php if (!$Page->incident_type_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fdisaster_incidentsadd", function() {
    var options = { name: "x_incident_type_id", selectId: "fdisaster_incidentsadd_x_incident_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdisaster_incidentsadd.lists.incident_type_id?.lookupOptions.length) {
        options.data = { id: "x_incident_type_id", form: "fdisaster_incidentsadd" };
    } else {
        options.ajax = { id: "x_incident_type_id", form: "fdisaster_incidentsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdisaster_incidentsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdisaster_incidentsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
