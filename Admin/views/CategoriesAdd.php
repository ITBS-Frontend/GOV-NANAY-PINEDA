...
<span id="el_categories_category_type_id">
    <select
        ...
        data-select2-id="fcategoriesadd_x_category_type_id"
        <?php } ?>
...
<?php if (!$Page->category_type_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fcategoriesadd", function() {
    var options = { name: "x_category_type_id", selectId: "fcategoriesadd_x_category_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcategoriesadd.lists.category_type_id?.lookupOptions.length) {
        options.data = { id: "x_category_type_id", form: "fcategoriesadd" };
    } else {
        options.ajax = { id: "x_category_type_id", form: "fcategoriesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
