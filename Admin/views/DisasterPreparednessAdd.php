...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdisaster_preparednessadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fdisaster_preparednessadd" id="fdisaster_preparednessadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_disaster_preparedness_disaster_type_id">
    <select
        ...
        data-select2-id="fdisaster_preparednessadd_x_disaster_type_id"
        <?php } ?>
        data-table="disaster_preparedness"
...
<?php if (!$Page->disaster_type_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fdisaster_preparednessadd", function() {
    var options = { name: "x_disaster_type_id", selectId: "fdisaster_preparednessadd_x_disaster_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdisaster_preparednessadd.lists.disaster_type_id?.lookupOptions.length) {
        options.data = { id: "x_disaster_type_id", form: "fdisaster_preparednessadd" };
    } else {
        options.ajax = { id: "x_disaster_type_id", form: "fdisaster_preparednessadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdisaster_preparednessadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdisaster_preparednessadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
