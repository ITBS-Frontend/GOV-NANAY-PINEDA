...
<span id="el_project_highlights_project_id">
    <select
        ...
        data-select2-id="fproject_highlightsadd_x_project_id"
        <?php } ?>
...
<?php if (!$Page->project_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fproject_highlightsadd", function() {
    var options = { name: "x_project_id", selectId: "fproject_highlightsadd_x_project_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fproject_highlightsadd.lists.project_id?.lookupOptions.length) {
        options.data = { id: "x_project_id", form: "fproject_highlightsadd" };
    } else {
        options.ajax = { id: "x_project_id", form: "fproject_highlightsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
