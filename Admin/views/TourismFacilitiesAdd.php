...
$Page->showMessage();
?>
<form name="ftourism_facilitiesadd" id="ftourism_facilitiesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_tourism_facilities_facility_type_id">
    <select
        ...
        data-select2-id="ftourism_facilitiesadd_x_facility_type_id"
        <?php } ?>
        data-table="tourism_facilities"
...
<?php if (!$Page->facility_type_id->IsNativeSelect) { ?>
<script>
loadjs.ready("ftourism_facilitiesadd", function() {
    var options = { name: "x_facility_type_id", selectId: "ftourism_facilitiesadd_x_facility_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftourism_facilitiesadd.lists.facility_type_id?.lookupOptions.length) {
        options.data = { id: "x_facility_type_id", form: "ftourism_facilitiesadd" };
    } else {
        options.ajax = { id: "x_facility_type_id", form: "ftourism_facilitiesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_tourism_facilities_ownership_type_id">
    <select
        ...
        data-select2-id="ftourism_facilitiesadd_x_ownership_type_id"
        <?php } ?>
        data-table="tourism_facilities"
...
<script>
loadjs.ready("ftourism_facilitiesadd", function() {
    var options = { name: "x_ownership_type_id", selectId: "ftourism_facilitiesadd_x_ownership_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftourism_facilitiesadd.lists.ownership_type_id?.lookupOptions.length) {
        options.data = { id: "x_ownership_type_id", form: "ftourism_facilitiesadd" };
    } else {
        options.ajax = { id: "x_ownership_type_id", form: "ftourism_facilitiesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftourism_facilitiesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftourism_facilitiesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
