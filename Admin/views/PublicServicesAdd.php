...
<span id="el_public_services_category_id">
    <select
        ...
        data-select2-id="fpublic_servicesadd_x_category_id"
        <?php } ?>
...
<?php if (!$Page->category_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fpublic_servicesadd", function() {
    var options = { name: "x_category_id", selectId: "fpublic_servicesadd_x_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpublic_servicesadd.lists.category_id?.lookupOptions.length) {
        options.data = { id: "x_category_id", form: "fpublic_servicesadd" };
    } else {
        options.ajax = { id: "x_category_id", form: "fpublic_servicesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
