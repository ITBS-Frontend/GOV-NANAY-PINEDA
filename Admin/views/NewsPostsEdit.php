...
<script>
loadjs.ready(["fnews_postsedit", "editor"], function() {
    ew.createEditor("fnews_postsedit", "x__content", 35, 4, <?= $Page->_content->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<span id="el_news_posts_news_type_id">
    <select
        ...
        data-select2-id="fnews_postsedit_x_news_type_id"
        <?php } ?>
        data-table="news_posts"
...
<script>
loadjs.ready("fnews_postsedit", function() {
    var options = { name: "x_news_type_id", selectId: "fnews_postsedit_x_news_type_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fnews_postsedit.lists.news_type_id?.lookupOptions.length) {
        options.data = { id: "x_news_type_id", form: "fnews_postsedit" };
    } else {
        options.ajax = { id: "x_news_type_id", form: "fnews_postsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("news_post_tags", explode(",", $Page->getCurrentDetailTable())) && $news_post_tags->DetailEdit) {
?>
