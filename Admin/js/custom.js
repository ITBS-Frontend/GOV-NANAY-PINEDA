window.addEventListener("load", function() {
    // Hide element by class or more specific attribute
    var elements = document.querySelectorAll(".dropdown-footer a.btn.btn-default");
    elements.forEach(function(el) {
        el.style.display = "none";
    });
    
    // Clear password field
    var passwordField = document.querySelector("#x__password");
    if (passwordField) {
        passwordField.value = '';
    }
});
...
(function () {
  ...
  let currentTheme = "lirmd";
  let initializationComplete = false;
  
  // IMPORTANT: Declare at module scope
  let toolbarBackup = null;
  let observer = null;
  ...
  const themes = {
    lirmd: {
      light: {
        "--body": "linear-gradient(180deg, #FFF 0%, #f5f1ed 100%)",
        "--navbar-border": "3px solid #2d5ec1",
        "--navbar": "#f5f1ed",
        "--main-sidebar": "linear-gradient(40deg, #CE1126 0%, #2d5ec1 100%)",
        "--metrics-card": "rgba(165, 129, 83, 0.15)",
        "--metrics-card-text": "rgb(26, 26, 26)",
        "--metrics-card-btn-text": "#2d5ec1",
        "--metric-card-icon-bg": "#A58153",
        "--metric-card-icon": "#ffffff",
        "--card-wrapper": "rgb(255, 255, 255)",
        "--card-border": "rgba(165, 129, 83, 0.3)",
        "--user-container-card": "rgba(165, 129, 83, 0.1)",
        "--menu-focus": "#c00248",
        "--menu-focus-treeview": "rgba(192, 2, 72, 0.5)",
        "--menu-focus-text": "#ffffff",
        "--menu-list": "rgb(255, 255, 255)",
        "--icon-menu": "#A58153",
        "--title-font": "rgb(45, 45, 45)",
        "--btn-text": "#A58153",
        "--btn-bg": "#a581535d",
        "--btn-hover": "#2d5ec1",
        ...
        "--btn-warning": "#c00248",
        "--btn-hover-primary": "#1e3f8a",
        "--btn-hover-info": "#8a6a45",
        "--btn-hover-success": "rgb(2, 66, 4)",
        "--btn-hover-warning": "#9a0138",
        "--add-btn-bg": "#2d5ec1",
        "--add-btn-hover": "#1e3f8a",
        "--view-btn-bg": "#A58153",
        "--view-btn-hover": "#8a6a45",
        "--edit-btn-bg": "#2d5ec1",
        "--edit-btn-hover": "#1e3f8a",
        "--delete-btn-bg": "#c00248",
        "--delete-btn-hover": "#9a0138",
        "--table-header": "#2d5ec1",
        "--table-footer-bg": "rgba(165, 129, 83, 0.15)",
      },
      dark: {
        "--body": "rgba(19, 25, 31, 1)",
        "--navbar": "rgba(35, 40, 50, 1)",
        "--main-sidebar": "linear-gradient(180deg, #1e3f8a 0%, #0f1f45 100%)",
        "--metrics-card": "rgba(165, 129, 83, 0.25)",
        "--metrics-card-text": "rgb(255, 255, 255)",
        "--metrics-card-btn-text": "rgb(255, 255, 255)",
        "--metric-card-icon-bg": "#A58153",
        "--metric-card-icon": "#ffffff",
        "--card-wrapper": "rgb(30, 35, 45)",
        "--card-border": "rgba(165, 129, 83, 0.4)",
        "--user-container-card": "rgba(45, 94, 193, 0.3)",
        "--menu-focus": "#c00248",
        "--menu-focus-treeview": "rgba(192, 2, 72, 0.5)",
        "--menu-focus-text": "#ffffff",
        "--menu-list": "rgb(255, 255, 255)",
        ...
        "--btn-bg": "rgba(165, 129, 83, 0.3)",
        "--btn-hover": "#2d5ec1",
        "--btn-primary": "#2d5ec1",
        "--btn-info": "#A58153",
        "--btn-success": "rgba(0, 150, 6, 1)",
        "--btn-warning": "#c00248",
        "--btn-hover-primary": "#3d6ed1",
        "--btn-hover-info": "#b59163",
        "--btn-hover-success": "rgb(0, 180, 8)",
        "--btn-hover-warning": "#d01258",
        "--btn-hover-text": "rgb(255, 255, 255)",
        "--add-btn-bg": "#2d5ec1",
        "--add-btn-hover": "#3d6ed1",
        "--view-btn-bg": "#A58153",
        "--view-btn-hover": "#b59163",
        "--edit-btn-bg": "#2d5ec1",
        "--edit-btn-hover": "#3d6ed1",
        "--delete-btn-bg": "#c00248",
        "--delete-btn-hover": "#d01258",
        "--table-header": "#1e3f8a",
        "--table-footer-bg": "rgba(45, 94, 193, 0.2)",
      },
    },
    blue: {
      light: {
        ...
        "--metrics-card": "linear-gradient(98deg, #081F62 0.54%, #003B7F 99.46%)",
        "--metrics-card-text": "rgb(255, 255, 255)",
        "--metrics-card-btn-text": "rgb(255, 255, 255)",
    ...
    fourthTheme: {
      ...
      dark: {
        ...
        "--delete-btn-bg": "rgb(220, 53, 69)",
        "--delete-btn-hover": "rgb(167, 26, 40)",
        "--table-header": "linear-gradient(98deg, #081F62 0.54%, #003B7F 99.46%)",
        "--table-footer-bg": "rgba(208, 214, 221, .2)",
      },
    },
  };
  ...
  const chartColors = {
    lirmd: {
      light: {
        borderColor: ["rgba(45, 94, 193, 0)"],
        gridColor: "rgba(0, 0, 0, 0.1)",
        textColor: "#2d5ec1",
      },
      dark: {
        borderColor: ["rgba(165, 129, 83, 0)"],
  ...
  // All DOM operations in one function
...
// Move toolbar with backup mechanism - UPDATED
function moveToolbar() {
  let toolbar = document.querySelector(".ew-toolbar");
  const toolbarTarget = document.querySelector(".ew-list-other-options");
  
  // If toolbar exists, ALWAYS update the backup (to capture any new buttons added by PHPMaker)
  if (toolbar) {
    toolbarBackup = toolbar.cloneNode(true);
    console.log("Toolbar backed up/updated");
  }
  
  // If toolbar doesn't exist but we have a backup, restore it
  if (!toolbar && toolbarBackup && toolbarTarget) {
    toolbar = toolbarBackup.cloneNode(true);
    console.log("Restoring toolbar from backup");
  }
   
  if (toolbar && toolbarTarget) {
    // Check if toolbar is not already in the target
    if (!toolbarTarget.contains(toolbar)) {
      toolbarTarget.appendChild(toolbar);
      console.log("Toolbar moved into ew-list-other-options successfully!");
    } else {
      console.log("Toolbar already in ew-list-other-options");
    }
  } else {
    console.log("Toolbar or target not found", {
      toolbar: !!toolbar, 
      toolbarTarget: !!toolbarTarget,
      hasBackup: !!toolbarBackup
    });
  }
}
  ...
  function updateButtonSymbols() {
    ...
    buttonUpdateTimeout = setTimeout(() => {
      ...
      if (addElement && !addElement.textContent.includes("✚")) {
        addElement.textContent = "✚ " + addElement.textContent;
      }
      if (editElement && !editElement.textContent.includes("✏️")) {
        editElement.textContent = "✏️ " + editElement.textContent;
      }
      if (deleteElement && !deleteElement.textContent.includes("🗑️")) {
        deleteElement.textContent = "🗑️ " + deleteElement.textContent;
      }
      if (copyElement && !copyElement.textContent.includes("📋")) {
      ...
      if (sourceElement && targetDiv) {
        if (!targetDiv.contains(sourceElement)) {
          ...
          console.log("Successfully moved ew-list-other-options to search container");
          // After moving, ensure toolbar is inside it
      ...
      // Always move toolbar after updates
      moveToolbar();
    }, 100);
  }

  // Setup PHPMaker AJAX event listeners
  function setupPHPMakerEventListeners() {
    if (typeof jQuery !== 'undefined') {
      // Monitor for filter form submissions
      jQuery(document).on('submit', '.ew-ext-search-form', function() {
        console.log("Filter form submitted");
        const checkTimes = [100, 300, 500, 1000, 1500];
        checkTimes.forEach(delay => {
          setTimeout(() => {
            console.log(`Checking toolbar at ${delay}ms`);
            moveToolbar();
            updateButtonSymbols();
          }, delay);
        });
      ...
      // Listen for jQuery AJAX complete events
      ...
      // Listen for changes in the column visibility dropdown
      jQuery(document).on('change', '.ew-list-other-options input[type="checkbox"]', function() {
        setTimeout(() => {
          moveToolbar();
        }, 100);
      });
    ...
    observer = new MutationObserver(function (mutations) {
      ...
      for (const mutation of mutations) {
        if (mutation.type === "childList") {
          ...
          for (const node of mutation.addedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              if (
                (node.matches &&
                  node.matches(
                    "a.ew-add, a.ew-edit, a.ew-delete, a.ew-copy, #ew-list, .ew-toolbar, .ew-list-other-options"
                  )) ||
                (node.querySelector &&
                  node.querySelector(
                    "a.ew-add, a.ew-edit, a.ew-delete, a.ew-copy, #ew-list, .ew-toolbar, .ew-list-other-options"
          ...
          // Check for removed nodes - if toolbar was removed, we need to restore it
          for (const node of mutation.removedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              if (
                (node.matches && node.matches(".ew-toolbar")) ||
                (node.querySelector && node.querySelector(".ew-toolbar"))
              ) {
                needsUpdate = true;
              }
            }
  ...
  // Aggressive watcher to keep restoring toolbar
  function startToolbarWatcher() {
    setInterval(() => {
      const toolbar = document.querySelector(".ew-toolbar");
      const toolbarTarget = document.querySelector(".ew-list-other-options");
      
      // If target exists but toolbar is missing, restore it
      if (!toolbar && toolbarTarget && toolbarBackup) {
        console.log("Watcher: Toolbar missing, restoring...");
        moveToolbar();
      } else if (toolbar && toolbarTarget && !toolbarTarget.contains(toolbar)) {
        console.log("Watcher: Toolbar exists but not in target, moving...");
        moveToolbar();
      }
    }, 500);
  }

  // Create collapsible controls
            ...
            <div class="controls-section">
                ...
                <div>
                    ...
                    <div style="display: flex; gap: 10px;">
                        <button class="color-theme-btn" data-theme="lirmd" style="width: 30px; height: 30px; border-radius: 50%; border: none; background: linear-gradient(135deg, #2d5ec1 33%, #A58153 33%, #A58153 66%, #c00248 66%); cursor: pointer;"></button>
  ...
  function setupEventListeners() {
    ...
    // Setup PHPMaker AJAX listeners
    setupPHPMakerEventListeners();

    // Start the toolbar watcher as a failsafe
    startToolbarWatcher();
  }
  ...
  function loadSavedPreferences() {
    const savedTheme = localStorage.getItem("phpmaker-theme") || "lirmd";
    const savedMode =
  ...
  function injectCSS() {
    ...
    style.textContent = `
            ...
            body { font-size: calc(14px * var(--body-scale)) !important; }
