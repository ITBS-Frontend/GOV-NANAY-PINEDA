    ...
    <main class="government-page">
        ...
        <section id="services" class="services-section">
            <div class="container">
                ...
                <div class="facility-filters" id="serviceCategories">
                    <div class="loading-spinner"></div>
                </div>
...
<section id="facilities" class="facilities-section">
    <div class="container">
        ...
        <!-- Facility Type Filter - Dynamic Loading -->
        <div class="facility-filters" id="facilityFilters">
            <div class="loading-spinner"></div>
        </div>
...
$(document).ready(function() {
    ...
    // Category chip click (for services)
    ...
    function loadFacilityTypes() {
        $.ajax({
            url: `${API_BASE}/Admin/api/government/facility-types`,
            method: 'GET',
            success: function(response) {
                if (response.success && response.data) {
                    renderFacilityTypeFilters(response.data);
                }
            },
            error: function() {
                console.log('Failed to load facility types, using default filters');
            }
        });
    }

function renderFacilityTypeFilters(types) {
    // Map type names to icons
    const iconMap = {
        'Hospital': 'fas fa-hospital',
        'School': 'fas fa-school',
        'Government Office': 'fas fa-building',
        'Barangay Hall': 'fas fa-landmark'
    };
    ...
    types.forEach(type => {
        ...
        const icon = type.icon_class || iconMap[type.type_name] || 'fas fa-building';
        html += `
            <button class="filter-btn" data-type="${type.id}">
                <i class="${icon}"></i>
    ...
    $('#facilityFilters').html(html);
}
    
    // Facility type filter (for facilities)
    ...
    function renderFacilities(facilities) {
        ...
        facilities.forEach(facility => {
            ...
            const typeMap = {
                'Hospital': { icon: 'fas fa-hospital', color: '#EF4444' },
                'School': { icon: 'fas fa-school', color: '#F59E0B' },
                'Government Office': { icon: 'fas fa-building', color: '#3B82F6' },
                'Barangay Hall': { icon: 'fas fa-landmark', color: '#10B981' }
            };
            
            // Get icon and color from API or fallback to typeMap
            const icon = facility.icon_class || typeMap[facility.facility_type]?.icon || 'fas fa-building';
            const color = typeMap[facility.facility_type]?.color || '#3B82F6';
